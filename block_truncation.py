...
if __name__ == '__main__':
    ...
    while True:
        ...
        #print(frame)
        #print(type(frame), type(out_frame))
        ...
        for i in range(m):
            for j in range(n):
                SE = SE+(int(frame[i][j])-int(out_frame[i][j]))**2
        MSE = SE/(m*n)
        ...
        out_frame = cv2.blur(out_frame, (5, 5))    #Tweak in this parameter to adjust blur (is always odd - (o,o))
        
        
        cv2.imshow('Input Video Feed', frame)
        cv2.imshow('Output Video Feed', out_frame)

        if cv2.waitKey(1) == ord('q'):
            break
    
    print("\n\n\nCompression ratio using Block Truncation is = 4:1\n\n\n")
